...
## 2024-05-24 - [User Enumeration Timing Vulnerability in Login]
**Vulnerability:** The `/login` endpoint returned immediately when a user was not found, but performed a slow `bcrypt.compare` when the user existed, allowing attackers to enumerate valid users via timing differences.
**Learning:** Returning generic error messages (like 'Invalid email or password') is not sufficient to prevent user enumeration if the processing time between the two failure paths (user not found vs incorrect password) differs significantly.
**Prevention:** Always perform a constant-time dummy operation (e.g., `bcrypt.compare` against a pre-generated valid dummy hash) in the 'user not found' path to pad the execution time and eliminate timing discrepancies. Generate the dummy hash synchronously at module load to avoid blocking the event loop during requests.
