...
## 2025-03-08 - User Enumeration Timing Attack in Login Flow
**Vulnerability:** The `/login` endpoint returned quickly with an "Invalid credentials" error when a user was not found, but took significantly longer to respond with "Invalid email or password" when the user existed but the password was incorrect, due to the expensive bcrypt comparison. This timing difference and varying error messages allowed attackers to enumerate valid usernames/emails.
**Learning:** Returning early or varying error messages based on whether an account exists exposes applications to user enumeration. The execution time of authentication endpoints must remain consistent regardless of whether the user exists.
**Prevention:**
1. Use identical generic error messages (e.g., "Invalid email or password") for both "user not found" and "incorrect password" cases.
2. Execute a constant-time dummy password hash comparison (`bcrypt.compare(password, DUMMY_HASH)`) when a user is not found to mimic the computational delay of a real password check.
3. Pre-generate the `DUMMY_HASH` once at the module level (using `bcrypt.hashSync('dummy', 10)`) during initialization to avoid blocking the event loop on every request and to prevent library formatting errors.
