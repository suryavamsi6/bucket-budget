...
## 2026-06-09 - User Enumeration via Timing Attack in Login
**Vulnerability:** The login endpoint was vulnerable to user enumeration via timing attacks. If a user was not found in the database, the endpoint returned immediately, whereas if the user existed, it performed a computationally expensive bcrypt comparison.
**Learning:** Attackers can measure the response times of login requests to determine whether an email or username exists in the system.
**Prevention:** Ensure constant-time processing during authentication flows. If a user is not found, always perform a dummy `bcrypt.compare` against a pre-generated hash before returning a generic error message (e.g., "Invalid email or password").
