...
## 2026-05-18 - Cross-Site Scripting (XSS) in AI Advisor
**Vulnerability:** A Cross-Site Scripting (XSS) vulnerability was found in `ui/src/pages/AiAdvisor.jsx`. The application used React's `dangerouslySetInnerHTML` to render AI responses and user messages without sanitizing the input beforehand, relying only on basic regex replacements for formatting. If malicious HTML or scripts were returned by the AI or entered by the user, they would be executed in the user's browser context.
**Learning:** React's `dangerouslySetInnerHTML` is exactly what its name implies: dangerous. Applying custom regex formatting directly on user/external content inside this property creates a high risk of XSS because standard characters (like `<` and `>`) are not escaped, allowing arbitrary DOM injection.
**Prevention:** Always escape HTML entities (`<`, `>`, `&`, `"`, `'`) in untrusted text *before* applying custom markup transformations (like bolding or line breaks), or use a dedicated markdown parsing library with built-in sanitization (e.g., `DOMPurify` combined with `marked`) instead of ad-hoc regex replacements.
