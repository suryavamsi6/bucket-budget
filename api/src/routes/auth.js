...
// Pre-compute a dummy hash for constant-time comparisons to mitigate timing attacks
...
export default async function authRoutes(fastify) {
    ...
    fastify.post('/login', async (request, reply) => {
        try {
            ...
            // To prevent timing attacks, always run bcrypt.compare
            let validPassword = false;
            if (user) {
                validPassword = await bcrypt.compare(password, user.password_hash);
            } else {
                // Dummy comparison taking roughly the same time
