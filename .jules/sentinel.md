...
## 2026-03-08 - Comprehensive SSRF Private IP Protection
**Vulnerability:** A previous mitigation for Server-Side Request Forgery (SSRF) in the `/api/ai/models` and `/api/ai/chat` endpoints (`api/src/routes/ai.js`) correctly filtered some cloud metadata IPs, but still allowed attackers to make proxy requests to private/internal network blocks (like `10.x.x.x`, `192.168.x.x`, `172.16.x.x` blocks, and IPv6 unique local / link-local addresses).
**Learning:** Checking against a strict array of known hardcoded metadata IPs is insufficient to fully protect against SSRF, as attackers can scan internal LAN addresses or containers on the Docker network where the application resides.
**Prevention:** Extend validation to comprehensively parse the hostname. Use string and numeric logic on parsed IP structures (`split('.')`) to statically reject addresses within IANA private IPv4 ranges (RFC 1918), local broadcast spaces (RFC 1122), and common IPv6 private ranges. Exceptions should explicitly target minimal necessary local ports for internal AI backends.
