...
## 2026-06-21 - User Enumeration via Timing Vulnerability
**Vulnerability:** The login endpoint performed `bcrypt.compare()` only if the user was found in the database. If the user was not found, it immediately returned an error. This allowed an attacker to enumerate valid users by measuring response times.
**Learning:** Conditional execution of slow operations like password hashing/comparison creates a timing side-channel.
**Prevention:** Always perform a dummy `bcrypt.compare()` when a user is not found to ensure the response time is consistent.
