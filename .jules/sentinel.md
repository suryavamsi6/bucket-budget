...
## 2026-06-18 - User Enumeration via Timing Attack in Login
**Vulnerability:** The `/login` endpoint returned immediately if a user was not found, but performed a time-consuming bcrypt comparison if the user existed but provided an incorrect password. This allowed attackers to enumerate valid usernames/emails by measuring response times. Error messages were also different.
**Learning:** To prevent timing-based user enumeration, authentication endpoints must take a consistent amount of time regardless of whether the user exists. Standardizing error messages alone is insufficient without constant-time processing.
**Prevention:** Pre-generate a valid dummy bcrypt hash on server startup (`bcrypt.hashSync('dummy', 10)`) and perform a dummy `bcrypt.compare` against it when a user is not found. Always return identical generic error messages (e.g., "Invalid credentials") for both conditions.
