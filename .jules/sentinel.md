...
## 2026-03-08 - Timing Attacks on Login Endpoints
**Vulnerability:** A timing attack vulnerability was identified in the login endpoint (`/api/auth/login`). When an invalid user identifier (email/username) was provided, the API returned a 401 response immediately. However, when a valid identifier was provided, it proceeded to compute a computationally expensive `bcrypt.compare` operation. This measurable time difference allowed attackers to enumerate existing users on the system.
**Learning:** Returning early upon an invalid user check leaks the existence of the user through response timing. Security endpoints must process invalid input and valid input in approximately constant time, especially when cryptographic functions are involved.
**Prevention:** Always perform a dummy `bcrypt.compare` using a pre-generated valid bcrypt hash with the same cost factor when a user is not found. This ensures that both successful and failed logins take roughly the same amount of computation time, masking user existence.
