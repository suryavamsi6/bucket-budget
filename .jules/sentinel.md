...
## 2024-05-27 - Timing Attack Vulnerability in Login
**Vulnerability:** The login endpoint checked if a user existed before comparing the password. If the user didn't exist, it immediately returned an error.
**Learning:** This introduces a timing vulnerability. An attacker can determine if an email/username is registered by observing the response time.
**Prevention:** To prevent user enumeration, login endpoints must use identical generic error messages (e.g., 'Invalid email or password') alongside constant-time processing (like a dummy bcrypt.compare) for both 'user not found' and 'invalid password' cases.
