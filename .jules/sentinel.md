...
## 2026-06-19 - Prevent User Enumeration via Timing Attacks in Login
**Vulnerability:** The `/login` endpoint returned immediately when a user was not found, but executed a time-consuming `bcrypt.compare` when the user was found but the password was wrong. This timing difference, along with different error messages, allowed attackers to enumerate existing users.
**Learning:** Returning immediately on user-not-found without simulating the expensive hashing operation introduces a side-channel timing vulnerability.
**Prevention:** Always ensure that authentication endpoints process requests in constant time and return standardized generic error messages (like "Invalid credentials") regardless of whether the user exists or the password is incorrect.
