...
## 2025-03-08 - User Enumeration via Timing Attack in Login
**Vulnerability:** The login endpoint immediately returned an error if a user was not found, skipping the computationally expensive `bcrypt.compare` operation. This resulted in a timing discrepancy, allowing an attacker to determine if an email or username exists in the system by measuring response times. It also returned a different error message ("Invalid credentials" vs "Invalid email or password").
**Learning:** Authentication endpoints must execute in near-constant time regardless of whether the user exists or not to prevent user enumeration. Additionally, identical error messages must be used for both "user not found" and "incorrect password".
**Prevention:** Generate a dummy bcrypt hash on module load. If a user is not found during login, execute `bcrypt.compare` using the provided password against the dummy hash to normalize the response time.
