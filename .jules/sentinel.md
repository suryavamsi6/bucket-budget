...
## 2026-06-30 - Timing Attack in Authentication
**Vulnerability:** The login endpoint was vulnerable to user enumeration via timing attack because it only evaluated the CPU-intensive `bcrypt.compare` function when a valid user record was found.
**Learning:** In authentication flows, the response time must be consistent regardless of whether the identifier exists in the database.
**Prevention:** Perform a dummy `bcrypt.compare` against a pre-generated valid hash whenever a user is not found, and ensure error messages are identically generic (e.g., 'Invalid credentials').
