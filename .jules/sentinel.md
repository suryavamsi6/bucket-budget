...
## 2024-07-04 - Timing Attack User Enumeration in Login Endpoint
**Vulnerability:** The `/login` endpoint returned quickly when a user was not found, but took longer to compare a password when a user was found. This timing discrepancy allowed attackers to enumerate existing users.
**Learning:** Different execution paths for authentication (user found vs. not found) must take roughly the same amount of time. An early return or distinct error message for missing users breaks constant-time execution.
**Prevention:** Use a dummy hash comparison (e.g., `bcrypt.compare`) whenever an invalid username is submitted so that the processing time remains constant. Always use a generic error message like 'Invalid email or password' for all authentication failures.
