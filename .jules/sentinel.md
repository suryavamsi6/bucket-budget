...
## 2024-05-24 - User Enumeration via Timing Attack in Login Endpoint
**Vulnerability:** The login endpoint in `api/src/routes/auth.js` does an early return `if (!user)` before executing `bcrypt.compare`. This allows an attacker to perform a timing attack to determine if a username/email exists in the database. Furthermore, it uses different error messages (`'Invalid credentials'` vs `'Invalid email or password'`) which directly allows user enumeration.
**Learning:** Early returns in authentication flows, while performant, can leak the existence of a user. Furthermore, error messages must be identical for both "user not found" and "invalid password" cases.
**Prevention:** Always use identical, generic error messages (e.g., 'Invalid email or password') and perform a constant-time operation (like a dummy `bcrypt.compare`) even if the user is not found to prevent timing-based user enumeration.
