...
## 2024-07-03 - Mitigate Timing Attack in Auth Login
**Vulnerability:** The `/login` endpoint returned early if a user was not found, making it susceptible to timing attacks that could allow user enumeration.
**Learning:** Returning early without performing a costly operation (like `bcrypt.compare`) allows attackers to differentiate between valid and invalid usernames based on response time.
**Prevention:** Always perform a dummy `bcrypt.compare` using a pre-generated `DUMMY_HASH` when a user is not found to normalize response times.
