...
## 2026-06-24 - User Enumeration via Timing Attack on Login
**Vulnerability:** The `/login` route returned early if a user was not found, bypassing the expensive `bcrypt.compare` operation. This allows an attacker to measure the response time and determine if an email or username exists in the system (user enumeration). It also returned a different error message ("Invalid email or password") when the password was wrong compared to when the user was not found ("Invalid credentials").
**Learning:** Returning early without performing a constant-time check leaks whether the user exists. Error messages must be indistinguishable between a bad username and a bad password.
**Prevention:** Always perform a dummy `bcrypt.compare` with a pre-generated valid hash when a user is not found, and always return a generic "Invalid credentials" error regardless of whether the user or password was wrong.
